...
nzv <- nearZeroVar(diff_matrix, saveMetrics = TRUE)

diff_matrix_cleaned <- diff_matrix[, !nzv$nzv]

#Models
lda.mod <- lda(Winner ~ . - ScoreDiff, data = diff_matrix_cleaned)
#lda.mod

preProc <- preProcess(diff_matrix_cleaned, method = "pca", thresh = 0.95)
diff_matrix_pca <- predict(preProc, diff_matrix_cleaned)
#diff_matrix_pca

qda.mod <- qda(Winner ~ ., data = diff_matrix_pca)
#qda.mod

#Predictions
# LDA
yhat_lda <- predict(lda.mod, newdata = diff_matrix_cleaned)
roc(predictor = yhat_lda$posterior[,1], response = diff_matrix_cleaned$Winner, plot = TRUE)
cm_lda <- confusionMatrix(yhat_lda$class, diff_matrix_cleaned$Winner, positive = "Home")

# QDA
yhat_qda <- predict(qda.mod, newdata = diff_matrix_pca)
roc(predictor = yhat_qda$posterior[,1], response = diff_matrix_pca$Winner, plot = TRUE)
cm_qda <- confusionMatrix(yhat_qda$class, diff_matrix_pca$Winner, positive = "Home")
...
lda_fit <- train(Winner ~ Fld..Diff.Field + BA.Diff.Off + OBP.Diff.Off + DefEff.Diff.Field + W.L..Diff.Pitch + SLG.Diff.Off + OPS.Diff.Off, data = diff_matrix_cleaned, method = "lda", trControl = control, metric = "ROC")

qda_fit <- train(Winner ~ ., data = diff_matrix_pca, method = "qda", trControl = control, metric = "ROC")
...
```{r}
mod.forest.class <- randomForest(Winner ~ . - ScoreDiff, data = diff_matrix_cleaned, importance = TRUE)

mod.forest.class



yhat_forest <- predict(mod.forest.class, newdata = diff_matrix_cleaned)
cm_forest <- confusionMatrix(yhat_forest, diff_matrix_cleaned$Winner, positive = "Home")
...
# Boosted Model
diff_matrix_cleaned01 <- diff_matrix_cleaned %>% mutate(Winner = if_else(Winner == "Home",1,0))
...
phat.boost.class <- predict(mod.boost.class, newdata = diff_matrix_cleaned01, n.trees = best.iter, type = "response")
yhat.boost.class <- factor(if_else(phat.boost.class > 0.5,"Home","Away"),levels = c("Home","Away"))
roc(predictor = phat.boost.class, response = diff_matrix_cleaned01$Winner, plot = TRUE)
confusionMatrix(yhat.boost.class,diff_matrix_cleaned$Winner, positive = "Home")
```
